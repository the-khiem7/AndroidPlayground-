...
public class SignInActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (com.example.lab10.data.mock.MockDataProvider.USE_MOCK_DATA) {
            com.example.lab10.data.local.TokenManager.get(getApplicationContext()).clear();
        }

        String token = com.example.lab10.data.local.TokenManager
                .get(getApplicationContext())
                .getAccessToken();

        if (token != null && !com.example.lab10.data.remote.JwtUtils.isExpired(token, 30)) {
    ...
    private void attemptLogin() {
        ...
        com.example.lab10.data.mock.MockApiWrapper.login(new LoginRequest(username, password))
